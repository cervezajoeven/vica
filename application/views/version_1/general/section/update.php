			...
			<div class="container">
				...
				<div class="form-group">
					...
					<select name="section[<?php echo $field?>]" class="form-control" required="" placeholder="Select">
						<?php foreach($general_class->data['all_data']['grade'] as $all_data_key=>$all_data_value): ?>
							<option <?php echo ($update_data['grade_id'] == $all_data_value['id']) ? 'selected' : false; ?> value="<?php echo $all_data_value['id']?>"> Grade <?php echo ucwords($all_data_value['grade_name']); ?></option>
						<?php endforeach; ?>
					</select>
				</div>
