...
<body>
  ...
  <!-- </pre> -->
  <div class="row">
    <div class="col-lg-7" id="holder">
      <div class="col-lg-12">
        <h3>Topic : <?php print_r($views_data['lesson_data']['lesson_name'])?></h3>
        <h3 style="display: inline;">Objective : </h3>
        <textarea class="form-control" id="objective" readonly="" style="display: inline;"><?php if($views_data['lesson_data']['objective']): ?><?php echo trim($views_data['lesson_data']['objective']); ?><?php endif; ?></textarea>
      </div>
    ...
    <div class="col-lg-5">
      <div class="col-lg-12">
        <button class="btn btn-success slideshow" style="display: none;">Slideshow</button>
      </div>
      <div class="col-lg-12">
        <h1>Preview</h1>
      </div>
    ...
    <div class="file-thumbnail-footer">
      <div class="file-actions">
          <div class="file-footer-buttons">
            ...
            <button type="button" id="" class="preview_button kv-file-zoom btn btn-kv btn-default btn-outline-secondary" folder="" file_name="" file_type="" title="Preview"><i class="glyphicon glyphicon-film"></i></button>
                
          </div>
          <input type="text" value="" readonly="" name="" lesson_id="<?php echo $views_data['lesson_id']?>" file_id="" class="thumbnail_text form-control" />
      </div>
...
<script type="text/javascript">
  ...
  function render_preview(data){
    ...
    if(data.file_type == 'image'){
      ...
      render.find("embed").show();
      render.find("embed").attr("src",files_path+data.folder+"/"+data.file_name);
      render.find("img").hide();
      render.find("video").remove();
    }
  }
  ...
  function render_file(render_data){
    ...
    if(render_data.file_type == 'image'){
      ...
      render.find("embed").attr("src",files_path+render_data.folder+"/"+render_data.file_name);
      render.attr("file_id",render_data.id);
      render.attr("folder",render_data.folder);
      render.attr("file_order",render_data.file_order);
      render.find("input").attr("file_id",render_data.id);
  ...
  $("#objective").focusout(function(){
    //$("#objective").val();
    var objective = $("#objective").val();
    $.ajax({
          type: "POST",
          url: update_objective,
          data: {id:lesson_id,objective:objective},
          success: function (result) {
            console.log(result);
