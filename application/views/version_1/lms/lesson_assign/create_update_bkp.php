<?php $data = $general_class->data; ?>
<?php $assigned_students = $general_class->data['assigned_students']; ?>
<?php $date_start = date('Y-m-d',strtotime($data['lesson_assign']['start_date'])); ?>
<?php $date_end = date('Y-m-d', strtotime($data['lesson_assign']['end_date'])); ?>
<?php echo $general_class->ben_open_form("lms/".$general_class->current_page['controller']."/save/".$data['lesson_id']); ?>

<div class="brainee-page_container col-lg-12">
	<div class="brainee-page_titlebar">
		<div class="brainee-page_titlebar_title">
			<span><?php echo $general_class->page_title ?> (<?php echo ucwords($general_class->current_page['function']); ?>)</span>
		</div>
		<div class="brainee-page_titlebar_controls_container">
			<a><button type="submit" class="btn btn-success" id="action_add">Assign</button></a>
			<a href="<?php echo $general_class->ben_link("lms/lesson/index/"); ?>"><button type="button" class="btn btn-danger" id="action_add">Cancel</button></a>
		</div>
	</div>
	

	<div class="brainee-page">
		<div class="container">
			<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/jstree/3.2.1/themes/default/style.min.css" />
			<input type="hidden" name="lesson_id" value="<?php echo $data['lesson_id']; ?>" />
			<input type="hidden" name="id" value="<?php echo $data['lesson_assign']['id']; ?>" />
			<div id="jstree_demo_div">
				<ul>
					<?php foreach ($data['grades'] as $grade_key => $grade): ?>
						<li>Grade <?php echo $grade['grade_name']; ?>
					        <ul>
					        	<?php foreach ($data['sections'] as $section_key => $section): ?>
					        		<?php if($section['grade_id']==$grade['id']): ?>
							          	<li><?php echo $section['section_name'] ?>
					        				
							          		<ul>
							          			<?php foreach ($data['classes'] as $class_key => $class): ?>
							          				<?php if($section['id']==$class['section_id']): ?>
							          					<?php if($class['account_id']): ?>
										          			<li <?php if(in_array($class['account_id'],$assigned_students )): ?>data-checkstate="checked" <?php endif; ?> data-jstree='{"value":<?php echo $class['account_id'];?>'><?php echo $class['first_name']; ?> <?php echo $class['last_name']; ?></li>
										          		<?php endif ?>
										          		
										          	<?php endif; ?>
									          	<?php endforeach; ?>
								      		</ul>
							      		</li>
						      		<?php endif; ?>
						      	<?php endforeach; ?>
					      	</ul>
				      	</li>
						
					<?php endforeach; ?>
			      	
			    </ul>
			    
			</div>
			<div class="form-group col-lg-12">
				<div class="form-group col-lg-3">
					<label>Start Date: </label>
				    <input type="date" name="start_date" id="start_date" value="<?php echo $date_start; ?>" class="form-control" />
				</div>
			</div>
			<div class="form-group col-lg-12">
				<div class="form-group col-lg-3">
					<label>End Date: </label>
				    <input type="date" name="end_date" id="end_date" value="<?php echo $date_end; ?>" class="form-control" />
				</div>
			</div>
			<div class="form-group">
			    <input type="hidden" value="" name="account_ids" id="account_ids" />
			    <input type="submit" value="Submit" class="form-control btn btn-primary" name="" id="submit" />
			</div>
		</div>	
	</div>
</div>
</form>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/1.12.1/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jstree/3.2.1/jstree.min.js"></script>
<script type="text/javascript">
	var checked_data = [];
	var tree = $('#jstree_demo_div').jstree({
		icon : "jstree-file",
		plugins : [
					"checkbox",
					"contextmenu",
					"dnd",
					"massload",
					"search",
					"sort",
					"state",
					"types",
					"unique",
					"wholerow",
					"changed",
					"conditionalselect"
		],
	});

	tree.on('changed.jstree', function (e, data) {
		var i, j, r = [];
		for(i = 0, j = data.selected.length; i < j; i++) {
			r.push(data.instance.get_node(data.selected[i]));
		}
		
		checked_data = r;
	}).on('ready.jstree', function (e, data) {
		// alert("asdkjaslkdj");
		tree.jstree('open_all');
		tree.jstree('deselect_all');
		$('li[data-checkstate="checked"]').each(function() {
		    tree.jstree('check_node', $(this));
		});
		tree.jstree('close_all');
	});

	$("form").submit(function(e){

		var student = [];
		var students = [];
		var street = [];
		$.each(checked_data,function(key,value){
			
			if(value.data.jstree){
				student[key] = value.data.jstree;
				street = JSON.parse(student[key]+'}');
				students.push(street.value);
			}

		});

		var start_date = $("#start_date").val();
		var end_date = $("#end_date").val();
		if(start_date>end_date){
			alert("Start Date must not be greater than End date.");
			e.preventDefault();
		}

		$("#account_ids").val(students.join(","));
		
	});
</script>