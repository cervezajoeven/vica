...
<li id="key_clone_matching_type_new" style="display: none" class="adding file-preview-frame ben-default ui-state-default kv-preview-thumb">
	<table class="table">
		<tr>
			<td>
				<div class="radio">
		...
		<tr>
			<td>
				<div class="form-group radio">
		            ...
		            <input type="number" id="key_clone_number_of_key" class="form-control" value="4" placeholder="4">
		        </div>
			</td>
...
<li id="matching_type_new_key" style="display: none;" class="file-preview-frame ben-default ui-state-default kv-preview-thumb">
	<table class="table">
		<tr id="first_row">
			<td id="number_container">
				<div class="radio">
...
<script type="text/javascript">
			...
			if(the_answers != ""){
				...
				$.each(the_answers,function(key,value){
					...
					if(object_key.includes("identification")){
						...
						the_score_array = value.the_score.split(",");

						$.each(the_score_array,function(key,value){

							$("#"+object_key).find(".the_score").eq(key).val(value);
						});
						$.each(teacher_answer,function(option_key,option_value){
	...
	function add_key(){
		...
		if(add_key_select=="identification"){
			...
			for(var count = 1;count<number_of_options;count++){
				...
				first_row.find("#action_element").find("button").remove();
				first_row.find("#action_element").removeAttr("id");
		...
		}else if(add_key_select=="matching_type_new"){
	...
	function render_matching_type_new(){
		...
		new_key.find("#the_letter").removeAttr("id");
		new_key.find(".the_score").eq(0).val("1");
		new_key.find("#number_container").removeAttr("id");
		new_key.find("#number").text(numbering);
		new_key.find("#number").remove("id");
	...
	$(document).on("click",".save",function(){
		...
		var quiz_id = "<?php echo $general_class->data['quiz'][0]['id']?>";
		...
		$.each($(".the_key"),function(key,value){
			if($(value).hasClass('multiple_choice')){
				...
				var holder = [];
				var score_array = [];
				$.each($(value).find(".the_score"),function(key,value){
					// var inputs_object = {[key]:$(value).val()};
					// holder.push(inputs_object);
					score_array.push($(value).val());
				...
				var the_score = score_array.join();
		...
		$.ajax({
	      	...
	      	data: {id:id,answer_key:answer_key,answers:answers,quiz_name:quiz_name,quiz_id:quiz_id},
	      	success: function (result) {
				...
				if(the_answers != ""){
					...
					$.each(the_answers,function(key,value){
						...
						$.each(the_actual_answers,function(the_actual_answers_key,the_actual_answers_value){
							...
							if(object_key == actual_object_key){
								...
								if(actual_object_key.includes("essay")||actual_object_key.includes("matching_type")){
					...
					$.ajax({
				      	type: "POST",
				      	url: optical_save_url,
				      	data: {id:id,answer_key:answer_key,answers:answers,total_score:total_score,quiz_name:quiz_name,quiz_id:quiz_id},
				      	success: function (result) {
				      		// console.log(result);
	      					if(result=="yes"){
...
<?php if($general_class->session->userdata('account_type_id')==5):?>
				...
				if(the_answers != ""){
					...
					$.ajax({
				      	...
				      	success: function (result) {
				      		// console.log(result);
	      					if(result=="yes"){
	      						console.log("Quiz was auto saved!");
	...
	setInterval(quiz_auto_save,30000);
	...
	function quiz_submit(){
		...
		var quiz_id = '<?php echo $general_class->data["optical"][0]['quiz_id']; ?>';
		var quiz_assign_id = '<?php echo $general_class->data["quiz_assign_id"]; ?>';
		var attempt_id = '<?php echo $general_class->data["attempt"][0]["id"]; ?>';
		$.each($(".the_key"),function(key,value){
			if($(value).hasClass('multiple_choice')){
				var radio_array = [];
				var radio_id = $(value).attr("id");
	      	...
	      	success: function (result) {
				...
				if(the_answers != ""){
					...
					$.ajax({
				      	type: "POST",
				      	url: optical_save_url,
				      	data: {id:id,account_id:account_id,quiz_id:quiz_id,answers:answers,quiz_assign_id:quiz_assign_id,score:the_real_score,attempt_id:attempt_id},
				      	success: function (result) {
	      					if(result=="yes"){
	      						alert("Quiz has been successfully submitted.");

				      			window.location.replace("<?php echo $general_class->ben_link('lms/optical/optical_review/'); ?>"+quiz_id+"/"+account_id);
				      		}else{
				      			alert("Not saved properly");
				      		}
				      	}
	...
	$(document).on("click",".time_up",function(){
		alert("This quiz will now be submitted!");
		quiz_submit();
